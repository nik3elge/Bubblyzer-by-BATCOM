...
- 💬 **Нативная интеграция с Affinity v2:** Скрипт `affinity_bubblyzer.js` экспортирует страницу, опрашивает локальный сервер и автоматически создает текстовые фреймы (`Frame Text`) точного размера прямо поверх найденных бабблов.
